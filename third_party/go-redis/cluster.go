...
import (
	...
	"github.com/charlie-bit/utils/third_party/go-redis/internal/hashtag"
...
type ClusterOptions struct {
	...
	// It can use server like ZooKeeper to maintain configuration information
...
func (c *ClusterClient) _processPipeline(ctx context.Context, cmds []Cmder) error {
	...
	return cmdsFirstErrWithoutNil(cmds)
}
...
func (c *ClusterClient) cmdSlot(cmd Cmder) int {
	...
	cmdInfo := c.cmdInfo(cmd.Name())
	if cmdInfo == nil {
		if "eval" == cmd.Name() || "evalsha" == cmd.Name() {
			return hashtag.Slot(cmd.stringArg(2))
		}
		return hashtag.Slot(cmd.stringArg(1))
	}
	return hashtag.Slot(cmd.stringArg(int(cmdInfo.FirstKeyPos)))
}
